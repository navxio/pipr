...
Getting Started (developer) 1. Clone repo 2. pnpm install 3. pnpm --filter @pipr/backend dev (or run the dev script for the monorepo) 4. Run migrations: pnpm --filter @pipr/backend prisma migrate dev 5. Seed sample project and start frontend 6. Run a simple agent locally against a small README ingestion pipeline
...
• add quickstart scripts to the monorepo root
• add sample ingestion pipeline with README/meeting note
• add example Mastra agent for “goal→tasks”
